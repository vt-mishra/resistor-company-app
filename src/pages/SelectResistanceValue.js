...
import backArrowImg from "./../assets/icons/homeArrow.svg";
import { Link } from "react-router-dom";
import { useNavigate } from "react-router-dom";
import React, { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";

const SelectResistanceValue = (props) => {
  const navigate = useNavigate();
  let findByResister = JSON.parse(localStorage.getItem("findByResister"));
  const [resistance, setResistance] = useState(findByResister.resistance);

  const onClickNextButton = () => {
    findByResister.resistance = resistance;
    localStorage.setItem("findByResister", JSON.stringify(findByResister));
    navigateTo("../select_tolerance");
  };
              ...
              <div className="row-B1">
                <div className="head-text">
                  <label className="page-head-lg">
                    {t("select_resistance_header_text_desktop")}
                  </label>
                  <label className="page-head-sm">
                    {t("select_resistance_header_text_mobile")}
                  </label>
                </div>
              </div>
...
export default SelectResistanceValue;
